...
const numbers = [10, 5, 25, 3, 40];

function findMinMaxES6(numbers: number [] ) : number  {
    const min = Math.min(...numbers);
    const max = Math.max(...numbers);
    return  min + max ;
}

const numbersArray = [10, 5, 25, 3, 40];
const resultES6 = findMinMaxES6(numbersArray);
console.log(resultES6); // יצג: { min: 3, max: 40 }
